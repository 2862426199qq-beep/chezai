...
int main(void) {
    ...
    printf("DHT11 test v4 on %s line %d\n\n", GPIO_CHIP, GPIO_OFFSET);
    ...
    for (int attempt = 0; attempt < 15; attempt++) {
        ...
        if (cksum == data[4]) {
            ...
            printf("Attempt %d: checksum err (%d.%d / %d.%d) ck=%d vs %d raw=[%02X %02X %02X %02X %02X]\n",
