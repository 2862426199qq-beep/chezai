...
static int create_gps_thread(void);
static int get_pos_dot(char *buf, unsigned char uc_cnt);

int gps_init()
{
    printf("gps_init: initializing rwlock...\n");
    fflush(stdout);
    
    // 始终初始化读写锁
    pthread_rwlock_init(&gps_rwlock, NULL);
    printf("gps_init: rwlock initialized\n");
    fflush(stdout);
    ...
    if (0>gps_fd)
    {
        printf("uart init error, GPS using default location\n");
        gps_valid = 1;
    }
    else if (uart_cfg(&cfg,gps_fd)) {
        close(gps_fd);
        gps_fd = -1;
        printf("uart cfg error, GPS using default location\n");
        gps_valid = 1;
    }
#else
    // DISABLE_HARDWARE模式，使用默认坐标
    printf("GPS disabled (DISABLE_HARDWARE mode), using default location\n");
    fflush(stdout);
    gps_valid = 1;
#endif
    saveData_location = 0;
    ...
    printf("gps_init: creating thread...\n");
    fflush(stdout);
    create_gps_thread();
    printf("gps_init: thread created\n");
    fflush(stdout);
...
#else
        // DISABLE_HARDWARE模式，使用默认坐标（已在静态变量中初始化）
        gps_valid = 1;
#endif
        usleep(500000);
    }
